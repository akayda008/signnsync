...
def extract_features(input_video_path, output_folder):
    ...
    with mp_holistic.Holistic(min_detection_confidence=0.5, min_tracking_confidence=0.5) as holistic:
        ...
        while cap.isOpened():
            ...
            # Default blank frames (if no detection occurs)
            face_frame = blank_frame.copy()
            right_hand_frame = blank_frame.copy()
            left_hand_frame = blank_frame.copy()
            ...
            # Write frames (either extracted or blank)
    ...
    for vid in [face_video_path, right_hand_video_path, left_hand_video_path]:
        if not os.path.exists(vid) or os.path.getsize(vid) == 0:
            print(f" Warning: Extracted video {vid} is empty or missing.")
            return
